...
void ray_color(t_cam r , t_mlx *mlx, int i, int j) {
	...
	t = 1;
	put_pixel(mlx, i, j, write_color(t ,init_rgb(1.0 - t + (t * 0.5), 1.0 - t + (t * 0.7), 1.0)));
	//put_pixel(mlx, i, j, write_color(t ,init_rgb(100, 100, 255)));
}
...
void	draw_background(t_mlx *mlx, t_meta meta)
{
	...
	double	v;
	//int		image_width = 400;
	//int		image_height = (int)((double)image_width / 16.0 / 9.0);
	for (int j = WIDTH-1; j >= 0; --j) {
		for (int i = 0; i < WIDTH; ++i) {
			...
			ray_color(r, mlx, i, j);
			//put_pixel(mlx, i, j, write_color(t ,color));
		}
	}
